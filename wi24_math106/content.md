...
Note: For all of the following exercises, assume that $k$ is infinite, and that $V = V_p$. 
...
* The fact that, if $f_1, \dotsc, f_r \in k[x_0, \dotsc, x_n]$ are homogeneous and if we identify $U_0$ with $\mathbb{A}^n$, then $V_p(f_1, \dotsc, f_r) \cap U_0 = V_a(g_1, \dotsc, g_r)$ where $g_i \in k[x_1, \dotsc, x_n]$ is the dehomogenization of $f_i$ with respect to $x_0$. 
...
1. Construct an explicit example of an ideal $I \subseteq \R[x_1, \dotsc, x_n]$ such that $V_a(I) = \emptyset$ but $V_p(I^h) \neq \emptyset$. Note: Do you see why it is crucial for constructing such an example that we be working over $\mathbb{R}$? 
1.  a. Prove that $(fg)^h = f^h g^h$ in $k[x_0, \dotsc, x_n]$ for any pair of polynomials $f, g \in k[x_1, \dotsc, x_n]$. 
...
* Nullstellensatz: If $k$ is algebraically closed, then $I(V(I)) = \sqrt{I}$ for any homogeneous ideal $I \subseteq k[x_0, \dotsc, x_n]$ such that $V(I) \subseteq \mathbb{P}^n$ is nonempty. 
...
Possible reading: {{ clo15 | replace: "x", "Section 8.3 starting from Theorem 8.3.8" }} and/or {{ ful08 | replace: "x", "Section 4.2" }}.
