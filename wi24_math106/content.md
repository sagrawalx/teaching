...
1. Suppose $f \in k[x]$ and let $f = a f_1^{m_1} \cdots f_r^{m_r}$ be the unique factorization of $f$ where $a \in k$ is the leading coefficient of $f$, each $f_i$ is monic and irreducible, and each $m_i$ is a positive integer. Define the *reduction* of $f$, denoted $f_{\mathrm{red}}$, by $$ f_{\mathrm{red}} = cf_1 \cdots f_r. $$
...
1. Let $S$ be the (infinite) set of all monomials in $k[x_1, \dotsc, x_n]$. Find a finite generating set for the ideal $I$ generated by $S$.
...
1. Prove that the number of monomials in $k[x_1, \dotsc, x_n]$ of total degree at most $d$ is $\binom{n+d}{d}$. 
1. Let $R$ be a ring and let $I \subseteq R[x]$ be an ideal in a single variable polynomial ring over $R$. Let $J$ be the subset of $R$ consisting of leading coefficients of elements of $I$. 
    a. Verify that $J$ is an ideal of $R$. Note: This fact is used in the proof of the Hilbert basis theorem. 
    ...
    a. For an integer $m \geq 0$, let $J_m$ be the subset of $R$ consisting of leading coefficients of elements of $I$ whose degree is at most $m$. Verify that $$J_0 \subseteq J_1 \subseteq J_2 \subseteq \cdots $$ and that $J_m$ is an ideal of $R$. Note: This fact is also used in the proof of the Hilbert basis theorem. 
