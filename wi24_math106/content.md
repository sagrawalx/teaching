...
1. Fix integers $m, n \geq 2$ and let $\phi : \mathbb{A}^1 \to \mathbb{A}^3$ be the polynomial map given by $\phi(t) = (t, t^m, t^n)$ and let $V$ be its image. 
...
* Definition of "$f(P) = 0$" for $f \in k[x_0, \dotsc, x_n]$ and $P = (a_0 : \cdots : a_n) \in \mathbb{P}^n$. Namely, we say that $f(P) = 0$ if and only if $f(a_0, \dotsc, a_n) = 0$ for *every* coordinate representation $(a_0 : \cdots : a_n)$ of $P$. 
